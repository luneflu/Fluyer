<script lang="ts">
	...
	const onScroll = () => {
		...
		const isAtBottom =
			logsContainerRef.scrollHeight - logsContainerRef.scrollTop <=
			logsContainerRef.clientHeight + 10;
...
{#if isVisible}
	<div
		bind:this={panelRef}
		class="fixed z-[100] flex h-80 w-96 flex-col border border-white bg-black text-white"
	...
	>
		...
		<div
			...
			class="flex-1 space-y-1 overflow-y-auto p-2 font-mono text-[10px]"
